...
import numpy as np
cimport numpy as np
from .pyx_utils import to_base_ndarray
...
def pyx_randint_choice(high, size=1, replace=True, p=None, exclusion=None):
    ...
    if p is not None:
        p = to_base_ndarray(p, dtype=np.float32)
        if p.ndim != 1:
    ...
    if isinstance(exclusion, (int, np.int32)):
        exclusion = [exclusion]
    ...
    results = np.zeros(size, dtype=np.int32)
...
def pyx_batch_randint_choice(high, size, replace=True, p=None, exclusion=None, thread_num=1):
    ...
    if thread_num < 1 or not isinstance(thread_num, (int, np.int32)):
    ...
    results = np.zeros(np.sum(size), dtype=np.int32)
